...
public class RubberBandSelect extends Component {
	...
	private HashSet<RubberBandListener> listeners = new HashSet<RubberBandListener>();
	private int width = 0;
	...
	private void drawRubberBand() {
		...
		if (endX > startX && endY > startY) {
			boundingBox = new Rectangle(startX, startY, endX - startX, endY
					- startY);
			...
			width = endX - startX;
			height = endY - startY;
			// UDTMgr.getMgr().sendStatusMessage( "Drawing Rect: " + "(X1=" +
			// startX + ",Y1=" + startY + ") (X2=" + endX + ",Y2=" + endY + ")"
			// );
		} else if (endX < startX && endY < startY) {
			boundingBox = new Rectangle(endX, endY, startX - endX, startY
					- endY);
			...
			boundingBox = new Rectangle(startX, endY, endX - startX, startY
					- endY);
			...
			boundingBox = new Rectangle(endX, startY, startX - endX, endY
					- startY);
	...
	public void setStartPoint(Point startPoint) {
		...
		// UDTMgr.getMgr().sendStatusMessage(
		// "RubberBandSelect--Started:  point is: X=" + startX + " Y=" + startY
		// );
		// drawRubberBand();
		// started = true;
	}
	...
	public void setEndPoint(Point endPoint) {
		...
		// UDTMgr.getMgr().sendStatusMessage(
		// "RubberBandSelect--Streching: points are: X=" + startX + " Y=" +
		// startY + " Ending Point is: X=" + endX + " Y="+ endY );
	...
	public void breakBand() {
		...
		hostComponent.repaint();
		// UDTMgr.getMgr().sendStatusMessage(
		// "RubberBandSelect-- Broke band, click to restart" );
	}
